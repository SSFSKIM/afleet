...
/// files on disk.
///
/// X9 and tracker entry 24: nothing this child writes may land inside a Claude Code config
/// home. A temporary directory is normally nowhere near one, but `TMPDIR` is settable, so the
/// resolved root is canonicalised and compared against every config home before a single
/// directory is created. When it does resolve inside one the initialiser throws `XCTSkip`
/// rather than writing.
struct TempTree {
