...
final class Rig: @unchecked Sendable {   // `lock` serialises every recorded array
    ...
    /// The suite's clock stepper, over this rig's manual clock. The body lives in `TestTiming`, so the facade's
    /// harness and this one cannot drift apart.
    ...
    typealias LockedFlag = TestTiming.LockedFlag
