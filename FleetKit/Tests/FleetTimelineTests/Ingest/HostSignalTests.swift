...
final class HostSignalTests: XCTestCase {
    ...
    func testPromptSentAttributesTheFollowingTurn() async throws {
        ...
        XCTAssertEqual(overlay.turns.last?.attribution, .prompted(uuid: uuid),
                       "an outstanding prompt claims the turn that follows it")
        await quiet()
        XCTAssertEqual(log.count, 0, "the tap's own fold publishes nothing: the overlay a channel renders is C6's")
        await ingestion.close()
        tap.finish()
    }
    ...
    func testASecondIdenticalAnswerIsIdempotent() async throws {
        ...
        let published = await logged(log, 1)
