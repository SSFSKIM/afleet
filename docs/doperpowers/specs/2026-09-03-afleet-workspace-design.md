...
- New channels mint their own UUID and pass `--session-id`; existing channels pass
  `--resume`; forks add `--fork-session`. A channel afleet created passes `--session-id` until its
  transcript exists and `--resume` from then on, decided from the engine's first record-carrying
  `transcript_mirror` or the index's registration and never from a count of prompts: the engine
  refuses `--session-id` for an existing transcript as already in use and creates no transcript
  before the first record (2.1.263 `cli.pretty.js:294952`; bundle §35.6.3; amended 2026-09-11 at
  tracker 162's closure). The launch flags only set initial state. Every
...
- **Diagnostics are metadata-only by default**: per frame, type, subtype, byte size, timing
  and request id, written to `~/Library/Logs/afleet/` as one metadata-only log per event domain
  (§11's table, 2026-09-07). Raw frame capture is a Developer
...
**Quit.** An owned channel is the engine on afleet's stdio, and the engine treats stream close
as wind-down (it kills every still-running local shell and abandons its other background
tasks), so an owned conversation cannot outlive afleet and no quit dialog can offer to keep one;
the way to keep a conversation is to release it first with *Open in terminal*. On
`applicationShouldTerminate`, if any owned channel has a turn running or running local shells,
afleet asks once, naming those channels, before ending them — the warning rule X9 already imposes
per channel for `end_session`; on confirmation, or when nothing is busy, it runs `terminate()` on
...
- **Markdown** through Apple's swift-markdown into attributed text with native code
  highlighting; tables native; diagrams deferred behind a rendering seam (a mermaid fence renders
  as a code block, as the terminal does; amended 2026-09-09 at C6.1's merge, tracker entry names
  mermaid.js 11.16.1).
...
diffs — opened in the Files tab's Monaco through an emitted `.diff` link, so the Source Control
panel imports no editor (ruled 2026-09-09; worded here at C7.7's merge); working-tree changes with
diffs against HEAD. Uses the `git` binary from the
...
and differential work never waits on the app target. Dependencies: libghostty-spm, swift-markdown, HighlightKit (PhraseHQ, MIT, pinned exactly; a
pure-Swift port of highlight.js chosen at C6.1's grill for token fidelity with the engine's own
highlighter, behind a live unhighlighted fallback — added 2026-09-09 at C6.1's merge), Monaco (build
...
| `~/Library/Logs/afleet/diagnostics.log` | ClaudeWire's metadata-only diagnostics: frame type, subtype, size, timing, request id, control answer behavior and classification without payload | rotated by its writer; the 50 MB budget applies across the four logs, oldest rotated files removed first |
| `~/Library/Logs/afleet/fleet.log` | FleetSessions' metadata-only notices: lifecycle and ownership events, verb exits, precondition verdicts | same writer-owned rotation, same directory budget |
| `~/Library/Logs/afleet/timeline.log` | C3's `TimelineNotice`s, sink composed by the app | same |
| `~/Library/Logs/afleet/app.log` | the app's own notices (launch sequence, registration, notifications) | same; *Delete diagnostics* clears all four (amended 2026-09-07 at C5's merge: one writer per file, so no two sinks ever share a path) |
...
must scrub is still a disclosure when it is committed as the input. The one identifier of a
person that is committed on purpose is the `review.reviewer` handle `make sign` writes into a
fixture: it is the human attestation this section's review gate exists to record, the same
identity git's author field carries on every commit, and neither an engine byte nor a path or
account identifier (ruled 2026-09-07 at C5's merge, its tracker entry 79). Two correct principles
...
- Untrusted text the engine relays — a subagent's output, a tool result, a file's contents — is
  sanitised by the timeline renderer before it reaches attributed text (bidi overrides, control
  characters and unbounded runs), and the sanitiser is C6.1's (added 2026-09-09 at C6.1's merge).
...
- **Required:** required.
- **Status:** cut landed 2026-09-07 as `2026-09-07-c6-conversation-surface.md` (four leaves,
  two waves); all four merged by 2026-09-10; **integration review completed** 2026-09-10 on
  `main` (two review lenses, eleven implemented corrections and one deferred benchmark workload,
  two correctives, the lineage check); **acceptance open**. Item 3 is *New channel*: tracker 162
  and 314 closed at `493c948` — creation, the isolation consumer, the launch-flag transition and the
  trust-review pane are implemented and verified headless; item 3's live title, item 47's real dialog
  and the isolation-dependent permission legs remain to be witnessed. Closure requires every
  inherited acceptance clause on the integrated path, verified acceptance-affecting corrections,
  actual required human/live witnesses with X9, and the reviewed/signed `send-message-delivery`
  fixture with verification and non-skipping replay. 397/428 are closed by verified merge `b2f1607`;
  435 is closed by verified merge `749246b` (relay conclusions at publish and release); 427 is deferred production item-builder
  performance coverage, not measured by S7. See the composite's Outcomes for closure details;
  a checklist walk and signature alone are insufficient.
...
- **X6 Store namespaces.** A namespaced key-value API with atomic writes and a schema
  ...
  computation consume is its own whichever layer writes it, which is why `sidebar.grouping`
  and `channels.unread` sit in the `fleetKit` namespace and C5 reads them rather than
  duplicating them (ruled at C5's merge, 2026-09-07). Owner: C4. Binds C5, C7. Amended
...
| C6 Conversation surface and Agents panel | composite spec `2026-09-07-c6-conversation-surface.md` (four leaves, its own tracking map) | cut landed 2026-09-07 after C5's merge (`78303c7`), approved by the human 2026-09-08; Y1 skeleton on `main` at `5e24f1a`; C6.1 Timeline renderer **merged** 2026-09-09 at `947c7cc` (S7 met, p99 7.66 ms); C6.2 Composer and header **merged** 2026-09-08 at `c2dae0f` (Y6 named); C6.3 Decision cards and threads **merged** 2026-09-09 at `0fe2797` (Y3 corrected, §8.4 corrected); C6.4 Agents panel **merged** 2026-09-10 at `19b4127` (71 commits; Y8 named; the C3 corrective `b412f2e` for its tree sources) — all four leaves merged; **integration review completed** 2026-09-10 on `main` `91e0902` (historical floor 2,501 executed, 30 skipped, 0 failures; correctives `91e0902` and `ec054f6`; eleven implemented corrections, one deferred benchmark workload; lineage check in the composite); **acceptance open** for the human/live witnesses and the fixture signature; tracker 162/314 closed at `493c948` (New channel, isolation consumer, trust-review pane; 2,589 total tests, 30 skipped, 0 failed); 397/428 closed at `b2f1607` (2,506 total tests, 30 skipped, 0 failed), 435 closed at `749246b` (2,516 total tests, 30 skipped, 0 failed), 427 deferred performance coverage (S7 measures markdown hosting, not production item builders). Full closure conditions in §17.4 C6 and composite Outcomes; human/live witnesses and a signed, verified, non-skipping relay fixture alone do not close missing implementation |
...
- 2026-09-07 reconciliation of C5 (merge `78303c7` from `child/c5-app-shell` `4c4ede4`, 212
  commits). §11's owned-files table now names one metadata-only log per event domain under one
  directory with the 50 MB budget across it (C5's second filing; one writer per file made the
  interleaving question moot and the unlink fault became `c31bebd`). X7 carries the child's
  amendment verbatim: a capability-carrying `ChannelContext` with no lifecycle object, the tab id
  set closed at seven, retained per-(tab, channel) sessions under an LRU, `unregister` async so a
  withdrawal never outlives its replacement. X6 states why `sidebar.grouping` and `channels.unread`
  are FleetKit's. §7.4 gains the *Quit* clause. The setup screen is §6.5's version gate plus C4's
  signed-out state, not a new screen; the sidebar's thirty-day split is §7.4 and §8.2 applied at
  scale. First paint is 2,505 ms median with no persisted snapshot and 2,121 ms with one, warm page cache, over 4,261 transcripts against the 5,000 ms budget; the cold case is unmeasured. Merge-time review: an independent panel (one sweep, five derived lenses, a verifier) confirmed fifteen findings — a crash opening a transcript through a symlinked config home, a firmlink-alias gap in the write-root guard, and thirteen ordering, release and correctness items; fourteen were fixed in a four-worker wave with failing-first tests, one (the raw-capture toggle records nothing) deferred to a FleetKit seam — `Fleet.init` taking a `capture:` — landed on `main` as a follow-up; a confirming round found one narrow gap in the new guard, fixed at `4c4ede4`. Leak-risk review clean on six checks. Two facts worth keeping: `open(2)` on a user-content directory is TCC-gated and blocks on the consent dialog, so the descriptor-based canonicaliser is scoped to the guard's own paths and every other caller stays on `realpath`; and every fixture and live gate so far ran under a `CLAUDE_CONFIG_DIR` scratch home (tracker 70). C6's dispatch inherits tracker entries 71 (quit-time shutdown), 74 (owned actions) and 77 (roster-change signal from C4); the assertion-leak class in three package test targets (entry 75) is a `main` fix wave after this merge. Human decisions 2026-09-07, both asked at this merge: `com.afleet.app` with display name `afleet` is the shipping identifier, not a placeholder (it keys the notification centre, the keychain and Application Support, so it is now fixed); the C7 cut is approved, C7.1–C7.3 dispatch after C6.
- 2026-09-07 C6's decomposing run (`2026-09-07-c6-conversation-surface.md`): four leaves, not
  the three §17 C6 sketched — the renderer and the composer split on the gate's own criteria
  (different state owners, invariants, failure modes and verification), cards with threads and
  the Agents panel stay as sketched. The sketch was advisory and is superseded by the cut. Five
  contracts between the leaves (a row registry and directory ownership, one card component
  hosted by the timeline and by Activity, the X7 tab handoff, chip-to-run navigation as an
  app seam rather than a `WorkspaceLink` case, and everything through X5) are binding in that
...
- 2026-09-10 C6 integration review (composite Revision Note of the same date; `main` `91e0902`):
  twelve findings yielded eleven implemented corrections (`91e0902`, `ec054f6`) and one deferred
  benchmark workload (427). Item 43's Developer action is implemented; real-engine end-to-end
  evidence remains unavailable, not a second deferred finding. §17.9's C6 row and §17.4's C6 status
  distinguish completed review from open acceptance. Item 3 is §14 *New channel*, requiring an
  afleet-selected UUID/new transcript and first-turn AI title, not answer mapping. Tracker 162's
  missing creation path and isolation consumer remain a cross-owner prerequisite under C6
  acceptance: shell/lifecycle implementation, C6 integration verification, also for item 47's
  trust entry and isolation-dependent permission legs. These are implementation blockers, separate
  from organisation-policy-blocked live tests. The human checklist adds items 3, 47 and 43; no
  witness is claimed. 397/428 fixes are in progress on a sibling branch, not closed; 435 remains a
  relay-conclusion gap across rebuild without observation; 427 remains production item-builder
  performance coverage missing from the historical S7 markdown-hosting measurement. Closure
  requires the full inherited acceptance and the evidence specified in C6 Outcomes, not only
  signing `send-message-delivery` and walking the checklist.
- 2026-09-10 follow-up corrective `b2f1607` closes tracker 397 and 428 with failing-first
  regressions, clean independent review and a full test run (2,506 total, 30 skipped, zero failures).
  C6's implementation and live-evidence prerequisites remain open as recorded in its composite.
- 2026-09-11 corrective `749246b` closes tracker 435 (composite Revision Note of the same date):
  relay conclusions are taken when a channel publishes and when its model is released, not only
  when a surface reads them, so item 51's delivery states survive a rebuild without turn summaries.
  Three Opus review rounds, the last clean; residue filed as 451 and 452. Floor 2,516 executed,
  30 skipped, zero failures across 19 bundles. C6 acceptance stays open for tracker 162, the live
  witnesses and the fixture signature.
- 2026-09-11 corrective `493c948` closes tracker 162 and 314 (composite Revision Note of the same date):
  §14 item 3's *New channel* exists — the sheet, the pending row, `Fleet.create`, the isolation consumer,
  the `--session-id`→`--resume` transition — and item 47's trust review is X5's own verb opening a
  bare-`claude` pane with the re-read after its exit. §6.1 gains the transition sentence above. Trust
  keys on the common repository root and consent on the checkout, as the engine does. Three Opus review
  rounds, the last converged. Floor 2,589 executed, 30 skipped, 0 failed across 19
  bundles. C6 acceptance stays open for the live/human witnesses and the fixture signature; persisting a
  created-but-unsent channel across relaunch is an untaken product decision (tracker 448).
