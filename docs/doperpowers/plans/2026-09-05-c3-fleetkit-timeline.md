...
**Spec:** `docs/doperpowers/specs/2026-09-05-c3-fleetkit-timeline.md` v2.4 (child of `docs/doperpowers/specs/2026-09-03-afleet-workspace-design.md §17 C3`, parent-pin `ee94449`; the parent's §7.3 in full, §8.8's tree data model, §5's package edges and §17.5 X1, X3 (consumed), X4 (owned), X6 (the `IndexStorage` seam), X7 as amended 2026-09-05 (the recent-URL query), X8 and X9 bind this work).
...
**Interfaces:**
- Consumes: Tasks 1, 2, 5 (`SyntheticTranscript.queuedTurnLines` included); Task 9's `ProjectionComparison`; from `ClaudeWire`: `WireEvent`, `Frame`, `SystemFrame`, `TranscriptMirrorFrame`, `MirrorError`, `ExitStatus`, `ProcessEpoch`.
...
public actor StreamIngestion {
    ...
    /// Owns the ordering between the channel's tap and the file: starts a task consuming `events` into a buffer, reads the file,
    ...
    /// `effects`, one per frame. A second `open` on one actor is a programmer error (`precondition`).
    ...
    public func close()                                            // cancels the consuming task and finishes `effects`; every query still answers
...
- `close()`: cancel `tap`, finish `effects`; `projection`, `state`, `offsets`, `paths` and `rawRecord` keep answering.

- [ ] **Step 3: Tests**

`IngestionTests`, driving every frame through a synthetic tap: `SyntheticTap` (fileprivate in the test file) wraps `AsyncStream<WireEvent>.makeStream()` with `send(_ frame: Frame, epoch: ProcessEpoch = .first)`, `exited(epoch: ProcessEpoch = .first)` (an `ExitStatus.code(0, stderrTail: "")`) and `finish()` on the continuation, and `nextEffect()` awaits one element of `ingestion.effects`, so every assertion runs after the actor applied the frame; "send" below means `send` then `nextEffect()`. No test calls `apply`, `mirrorError` or `processExited` — they are internal — and none is timing-dependent: a frame sent before `open` is buffered and aligned, a frame sent after it is applied on arrival, and the assertions below hold either way (the `tapAligned` counts are what tell the two paths apart).
...
Run: `swift test --package-path FleetKit --filter IngestionTests 2>&1 | grep -E "Executed|failed"` → `Executed 16 tests, with 0 failures`.
