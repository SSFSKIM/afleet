...
- `swift test` redirected to a file is block-buffered, so a frozen log's last "started" line does not name the hanging test. Confirm a hang with `sample <pid>` on the live process, or by isolating the suspected suite under `timeout`.
- A break harness puts `swift test` in its own process group and kills the group on timeout: a plain subprocess timeout kills `swift-test` and orphans the `xctest` grandchild.
