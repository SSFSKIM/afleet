...
**Spec:** `docs/doperpowers/specs/2026-09-05-c4-fleetkit-sessions-fleet.md` v2.4 (child of `docs/doperpowers/specs/2026-09-03-afleet-workspace-design.md §17 C4`, parent-pin `ee94449`; the parent's §6.11, §6.12, §7.1, §7.2, §7.4, §7.6, §7.7, §7.8 and §17.5 X1, X2, X3, X5, X6, X9, X10 bind this work, X5 and X6 as amended on 2026-09-05). Conflicts found during execution resolve against the spec; a wrong binding clause flows back to the parent as `[parent-impact]`, never a local override.
...
**Plan revision v4** (2026-09-05), after the plan's third and final adversarial review (fifteen findings, all folded into spec v2.4). (1) `sendToBackground()` and `openInTerminal()` rerun `ownership.beforeSpawn` after the release wait and immediately before the verb or the pane request; a holder found there is the new `handoffPreempted` row (six scenarios) and throws `heldElsewhere` (Tasks 2, 5, 12). (2) `ProjectMCPServer` carries a `Transport` (stdio, http, sse, other) and an `entryHash` over the whole raw entry (Tasks 2, 7). (3) A fork stays `.connecting` until `.sessionIdentityResolved`, when the post-handshake check runs against the resolved id and the counter rekeys the slot (Tasks 4, 6). (4) The dormant `capReached` scenario is gone: a dormant channel holds no slot (Tasks 2, 4, 5). (5) Recency is the counter's fleet-wide `activityClock`; the per-supervisor `activitySequence` is gone (Tasks 4, 5). (6) A failed answer write consumes the id and throws `answerFailed`, mirroring `ClaudeProcess.answer` (Tasks 2, 3, 4). (7) `ProcessHandle.terminate()` returns a `TerminationReport` whose steps come from the `CapturingDiagnostics` sink every factory installs (Tasks 2, 4, 5). (8) `ChannelState.pendingDecisions` is `[PendingDecision]`, the Activity view's decision source (Tasks 2, 4, 9). (9) `jobs()` and `performJob(_:_:)` replace the job actions, `Holder.isJob` classifies by evidence, and the adoption live test sends the job back (Tasks 2, 3, 9, 10). (10) `LaunchConfiguration.maxTurns` is on `main` (`1458568`); Task 10 preflights it by content and passes explicit per-scenario caps. (11) The composed live scenario answers the Notification hook before the held permission and waits for the background shell's follow-up turn; the suite ceiling is four turns (Task 10). (12) Every type a later task or test uses is declared in Task 2: `SystemItem`, `PendingDecision`, `TerminationReport`, `JobEntry`, `JobVerb`, `ProjectMCPServer.Transport`, `LifecycleError.answerFailed` and `.logoutInProgress`; `ChannelState.identity` is ClaudeWire's `SessionIdentity`. (13) `CryptoKit` joins the import allowlist behind `ContentHash.sha256Hex`, pinned to one vector (Tasks 2, 9). (14) The symlinked-`.claude` clause of the decline test expects `symlink` (Task 7). (15) Coverage keys go through `testID()`, which strips the parentheses `#function` reports (Tasks 4, 12).
...
- Live tests run only with `AFLEET_LIVE_CLI=1` and skip with a named reason otherwise; the two turn-spending scenarios also need `AFLEET_LIVE_CLI_TURNS=1`. Every live scenario runs through the suite's one `LiveBudget` (Task 10): scenarios are serialised; the model is pinned to `claude-haiku-4-5-20251001` on every turn-spending launch; every `ClaudeProcess` the suite launches carries `--max-turns` with an explicit per-scenario cap (1 for handshake-only launches and the adoption resume, 12 for the composed scenario; the flag caps agentic turns inside one prompt, not prompts, and a `result` of subtype `error_max_turns` fails the scenario); the ceilings are four model turns (adoption 2, composed 2) and ten minutes of wall time for the whole suite; C2's usage reading is taken before the first scenario and before each turn-spending one, and a spent window skips with its reason; every `result` frame's `total_cost_usd` is summed and reported; a zero-cost scenario asks `get_session_cost` before ending its process and asserts `total_cost_usd == 0`, because a zero-turn launch emits no `result` frame.
...
- Every timer is driven by the injected `any Clock<Duration>`; no test sleeps on wall time to move the lifecycle; no instant comparisons in `FleetSessions` (durations are slept, recency is the cap counter's fleet-wide `activityClock`, a `UInt64` stamp).
...
FleetKit/
  ...
  Sources/FleetSessions/
    ...
    Types/ChannelState.swift                     ChannelKey, ChannelState, Presence, ChannelBanner, HeaderNote, EscalationTrace, TerminationReport, PendingDecision, SystemItem, DesiredOwnership, SessionRuntimeState (identity is ClaudeWire's SessionIdentity)
    Types/Actions.swift                          LifecycleAction (fork(at: ForkPoint?), answer(RequestID, InboundAnswer); no job verbs), JobVerb, JobEntry, SpawnPrecondition, ProjectMCPServer (with Transport), RestartRequest, RestartSnapshot (typealias), LifecycleError (with answerFailed, logoutInProgress)
    Types/Panes.swift                            PanePurpose, PaneRequest (with id), PaneExit, JobShort
    Support/ContentHash.swift                    ContentHash.sha256Hex over CryptoKit (the .mcp.json entry hash and the managed-settings payload hash)
    ...
    Lifecycle/ProcessHandle.swift                ProcessHandle protocol (terminate() -> TerminationReport), LiveProcessHandle (the wrapper over ClaudeProcess), CapturingDiagnostics (the DiagnosticsSink every factory installs), ProcessFactory
  ...
  Tests/FleetSessionsTests/
    ...
    LifecycleTableTests.swift, DormantEligibilityTests.swift, ListingPolicyTests.swift, ContentHashTests.swift   Task 2
    RecordsTests.swift, HolderReaderTests.swift, FleetObserverTests.swift, CLIVerbsTests.swift, OriginResolverTests.swift   Task 3
...
- Create: `FleetKit/Sources/FleetSessions/Support/ContentHash.swift`
- Test: `FleetKit/Tests/FleetSessionsTests/LifecycleTableTests.swift`, `DormantEligibilityTests.swift`, `ListingPolicyTests.swift`, `ContentHashTests.swift`
...
- Produces: every X5 value type exactly as the spec's *Types* block (including `PaneRequest.id`, `SessionRuntimeState`, `LifecycleAction.fork(at: ForkPoint?)`, `LifecycleAction.answer(RequestID, InboundAnswer)`, `LifecycleError.decisionGone`, `.answerFailed`, `.logoutInProgress`, `PendingDecision`, `SystemItem`, `TerminationReport`, `JobEntry`, `JobVerb`, `ProjectMCPServer.Transport`), `ContentHash.sha256Hex`, `LifecycleTable.Row`, `LifecycleTable.Event`, `LifecycleTable.Transition`, `LifecycleTable.scenarios`, `DormantEligibility.evaluate`, `TaskMirrorReading` (with `isArmed`), `MirrorEntryStandIn`, `ListingPolicy.include`; Tasks 4–9 build on them.
...
- `testSHA256HexMatchesAPinnedVector`: the bytes `{"args":["-y","@example/marker-server"],"command":"npx","env":{"MARKER_DIR":"/tmp/marker"}}` (an invented entry in the canonical form Task 7 hashes: sorted keys, no whitespace, slashes unescaped) hash to `4870945a32634bf7de261a41193d9cbbf330c8f0a086d7f463703d27802e6b02` (computed at plan time with `printf '%s' '<those bytes>' | shasum -a 256`); the same test canonicalises the entry from a literal with the keys in another order through `JSONSerialization.data(withJSONObject:options: [.sortedKeys, .withoutEscapingSlashes])` and asserts the bytes equal the vector's input (Foundation escapes `/` unless told not to, which is why the option is named). Deliberate break: drop `.withoutEscapingSlashes` → the canonical bytes differ and the digest does not match.

- [ ] **Step 3: Implement the value types**

`Types/ChannelState.swift`, `Types/Actions.swift`, `Types/Panes.swift`, `Types/LifecycleAPI.swift`: exactly the spec's *Types* block (`ChannelKey`, `DesiredOwnership`, `Holder`, `HolderSet`, `ChannelState`, `Presence`, `ForeignPresence`, `EscalationTrace`, `SpawnPrecondition`, `ProjectMCPServer` with its nested `Transport`, `LifecycleAction`, `LifecycleError`, `RestartRequest`, `RestartSnapshot`, `TerminationReport`, `PendingDecision`, `SystemItem`, `JobEntry`, `JobVerb`, `AgentsRow` goes to Task 3, `JobShort`, `PanePurpose`, `PaneRequest`, `PaneExit`, `LifecycleAPI`), each with a public memberwise initialiser written out. `ChannelState.identity` is ClaudeWire's `SessionIdentity` (`ClaudeWire/Sources/WireTransport/WireEvent.swift:78-91`, the two cases the spec shows: `known(SessionID)`, `awaitingFork(from:provisional:)`); FleetSessions declares no duplicate, which would be ambiguous under `@testable import FleetSessions` beside `import ClaudeWire`. No later task modifies a `Types/` file; a task that had to would say so in its Files list, and none in this plan does. Additions the spec names in prose:
...
public enum LifecycleError: Error, Hashable, Sendable {
    ...
    case answerFailed(RequestID, reason: String)  // the write behind an answer failed; the id is consumed (ClaudeProcess.answer removes pendingInbound[id] before it writes)
    case logoutInProgress                         // Task 8's barrier
}

public struct TerminationReport: Hashable, Sendable {          // ProcessHandle.terminate()'s result
    public var exit: ExitStatus?                               // nil: the escalation ran out (wedged)
    public var steps: [String]                                 // ClaudeWire's terminateEscalated steps for this epoch, in order
    public init(exit: ExitStatus?, steps: [String])
}
public struct PendingDecision: Hashable, Sendable {            // one per surfaced InboundRequest, in ask order
    public var id: RequestID
    public var subtype: String                                 // can_use_tool, hook_callback, ... (from the request's payload)
    public var epoch: ProcessEpoch
    public var askedAt: Date                                   // stamped by the supervisor when the frame arrived
    public init(id: RequestID, subtype: String, epoch: ProcessEpoch, askedAt: Date)
}
public enum SystemItem: Hashable, Sendable {                   // ExitStatus is Hashable on main (WireEvent.swift:47-51)
    case crashed(exit: ExitStatus, reopenOffered: Bool)
    case wedged(EscalationTrace, reopenOffered: Bool)
}
public struct JobEntry: Hashable, Sendable {                   // LifecycleAPI.jobs(): every roster job, exec jobs included
    public var short: JobShort
    public var state: String
    public var kind: String
    public var sessionID: SessionID?                           // nil for an exec job
    public var cwd: URL?
    public var name: String?
    public init(short: JobShort, state: String, kind: String, sessionID: SessionID?, cwd: URL?, name: String?)
}
public enum JobVerb: Hashable, Sendable { case stop, respawn, remove }   // LifecycleAPI.performJob(_:_:)
extension ProjectMCPServer {                                   // ProjectMCPServer { name; transport; entryHash }
...
`Support/ContentHash.swift`: `public enum ContentHash { public static func sha256Hex(_ data: Data) -> String }` over CryptoKit's `SHA256`, lowercase hex; Task 7's `.mcp.json` entry hash and managed-settings payload hash both go through it, and `CryptoKit` is on Task 9's import allowlist for this one file.
...
Run: `swift test --package-path FleetKit 2>&1 | grep -E "Executed|error:"`
Expected: `Executed 23 tests, with 0 failures` (8 from Task 1 plus 15 here).
...
- Test: `FleetKit/Tests/FleetSessionsTests/RecordsTests.swift`, `HolderReaderTests.swift`, `FleetObserverTests.swift`, `CLIVerbsTests.swift`, `OriginResolverTests.swift`
...
- `testAHolderMergedFromARegistryRecordAndARosterEntryCarriesTheJobShort`: a registry record and a roster entry for the same session and pid → one `Holder` with `sources == [.registry, .roster]`, `jobShort` set from the roster, `isJob == true`. Deliberate break: keep the registry holder's `jobShort` nil on merge → `isJob` is false and the resolver calls it a terminal.

`OriginResolverTests.swift` (pure, no files):
- `testAHolderMergedFromARegistryRecordAndARosterEntryIsABackgroundJob`: a `Holder` with `sources == [.registry, .roster]`, a live pid that is not ours and `jobShort` set → `OriginResolver.resolve(key:, ownedState: nil, holders: [h], pendingHatch: false)` is `.backgroundJob`; the same holder with `sources == [.registry]` and `jobShort == nil` → `.foreignLive(.usersTerminal)`; a session with a foreign holder and a job holder → foreign wins (the parent's precedence: owned, foreign, job, archived); no holder → archived. Deliberate break: classify by branch order alone (any live registry holder is foreign) → the merged holder resolves `.foreignLive`.
...
`Fleet/OriginResolver.swift`: `static func resolve(key:, ownedState: OwnedView?, holders: [Holder], pendingHatch: Bool) -> (ChannelOrigin, Presence)`, classifying each holder by evidence and ordering by the parent's precedence (owned, foreign, job, archived): owned when `ownedState` is non-nil; foreign live when a live holder with `isJob == false` that is not ours names the session (`.ownTerminalTab` when `pendingHatch`, else `.usersTerminal`); background job when a live holder with `isJob` names it (a holder merged from a registry record and a roster entry is a job, never a terminal); else archived. Presence for foreign from `ForeignPresence`, `.unknown` when absent; for owned from `OwnedView {turnRunning, pendingDecisions: Int (the count of `ChannelState.pendingDecisions`), sessionStateRequiresAction}`.
...
public enum FleetDiagnosticEvent: Sendable {
    ...
    case answerWriteFailed(id: RequestID, reason: String)         // Task 4: the engine write behind an answer failed; the id is consumed
...
Run: `swift test --package-path FleetKit 2>&1 | grep -E "Executed|error:"`
Expected: `Executed 43 tests, with 0 failures` (23 from before plus 20 here; adjust the number to the count actually written and quote it in the commit body).
...
**Interfaces:**
- Consumes: Task 2's types and table; Task 3's `FleetObserver`, `HolderReader`, `CLIVerbs`, `FleetDiagnosticsSink`, `TestClock`, `ScratchConfigHome`, `ScriptedHolderFiles`; `ClaudeWire`'s `ClaudeProcess`, `LaunchConfiguration`, `WireEvent`, `Handshake`, `ExitStatus`, `AfleetMCPServer`, `SendUserFileTool`, `DiagnosticsSink`, `DiagnosticEvent`, `NullDiagnostics`, `InboundRequest`, `RequestID`, `InboundAnswer`.
- Produces: `ProcessHandle` (`terminate() -> TerminationReport`), `LiveProcessHandle`, `CapturingDiagnostics`, `ProcessFactory`, `ChannelSupervisor` with `open()`, `send(_:)`, `answer(_:_:)`, `events() -> AsyncStream<WireEvent>`, `shutdown()`, `state`, `updates`, `reap()`, `handle(event:)`; `FleetCapCounter` with `acquire`, `confirm`, `rollback`, `release`, `rekey`, `setEligibility(_:_:)`, `noteActivity(_:)` over its `activityClock`, `markWedged`, `clearWedged`; `OwnershipCheck`; rows `archivedRecentOpened`, `archivedOlderOpened`, `archivedOlderSent`, `connectingClean`, `connectingFoundHolder`, `readyDormantEligible`, `dormantSent`, `dormantHolderAppeared`, `exitedNonZero` covered. Task 5 adds the rest.
...
`Support/ScriptedProcessHandle.swift`: `final class ScriptedProcessHandle: ProcessHandle, @unchecked Sendable` (lock-serialised) whose `spawn` returns a canned `Handshake`, whose `events` is an `AsyncStream<WireEvent>` whose continuation the test holds (`push(_ event: WireEvent)`, `finish()`; nothing outside ClaudeWire can construct or feed a `WireEventStream`, which is why the protocol's `events` is an existential), whose `terminate()` returns the `TerminationReport` the test set (`terminateReturns`; a `nil` exit with the scripted steps for the wedged rows), whose `answer(_:_:)` records the id in `consumed`, appends `(id, answer)` to `answers` and throws what the test scripted (`answerError`), consuming the id on failure too (the contract of `ClaudeProcess.answer`, which removes `pendingInbound[id]` before it writes, `main` ClaudeProcess.swift:396-399), and whose `childProcessIdentifier` is a value the test chooses. Used only where the plan says: the wedged rows, the decision tests, the fork collision.
...
`DecisionTests.swift` (the scripted handle throughout, via `rig.useScriptedHandle(terminateReturns: TerminationReport(exit: .code(0, stderrTail: ""), steps: []))`; the rig's scripted factory hands out a fresh handle per spawn and keeps them in `rig.scriptedHandles`; `allow` is one `InboundAnswer.permission` value the test builds; `r` is an `InboundRequest` built with ClaudeWire's public initialiser, id `d-1`, the handle's epoch, a `canUseTool` payload):
- `testAnswerResolvesAPendingDecisionOnlyAfterTheProcessAccepts`: ready; push `.request(r)` → `state.pendingDecisions.count == 1`, the entry carries `r.id`, `r`'s subtype and the current epoch, and the channel is not dormant-eligible; `perform(.answer("d-1", allow))` (the scripted `answer` succeeds) → the handle's `answers == [("d-1", allow)]` and `pendingDecisions.isEmpty`; an answer for an id never asked throws `decisionGone` and reaches no handle. Deliberate break: skip the `pendingDecisions` guard → the unknown id reaches the process.
- `testAnAnswerWhoseWriteFailsIsConsumedAndReported`: ready; push `.request(r)`; set `answerError`; `perform(.answer("d-1", allow))` throws `LifecycleError.answerFailed("d-1", reason:)` with `String(describing:)` of the scripted error; afterwards `pendingDecisions.isEmpty`, the handle's `consumed == ["d-1"]`, a second `perform(.answer("d-1", allow))` throws `decisionGone` and reaches no handle, the channel is dormant-eligible again, and the diagnostics recorded `.answerWriteFailed(id: "d-1", reason:)`. Deliberate break: keep the id pending on failure → the second answer reaches the process, which (like `ClaudeProcess`, whose `pendingInbound[id]` is already gone) throws `unknownRequest`, and the channel stays ineligible forever.
- `testAnExitEmptiesPendingDecisions`: ready; push `.request(r)`; push the exit the `exitedNonZero` test pushes → `pendingDecisions.isEmpty`; `perform(.answer(r.id, allow))` throws `decisionGone`. Deliberate break: keep the entries across an exit → an answer reaches a process that no longer exists.
- `testAnsweringACancelledDecisionThrowsDecisionGone`: push `.request(r)`, then `.requestCancelled("d-1", epoch)` → `pendingDecisions.isEmpty`; `perform(.answer("d-1", allow))` throws `LifecycleError.decisionGone("d-1")` and `answers` is empty. Deliberate break: forward unknown ids to the process → the handle recorded an answer for a request the engine withdrew.
- `testADuplicateAnswerThrowsDecisionGone`: answer `d-1` once (recorded), then again → `decisionGone`, `answers.count == 1`.
- `testAnAnswerFromAnOlderEpochThrowsDecisionGoneAfterARespawn`: push `.request(r)` in epoch 1, then `.exited(.code(1, stderrTail: ""), epoch 1)` and `finish()`; advance the clock past the 1 s backoff so the supervisor respawns (epoch 2, the second scripted handle); `perform(.answer("d-1", allow))` → `decisionGone`; neither handle recorded an answer and `pendingDecisions.isEmpty`. Deliberate break: keep `pendingDecisions` across an exit → the answer is forwarded to a process that never asked.
- `testTwoEventSubscribersSeeTheSameFrames`: two `supervisor.events()` streams taken before any push; push four events (three `.requestCancelled` with distinct ids, one `.request(r)`); both streams yield the same four events in order and `pendingDecisions.count == 1`; a third stream taken after the pushes sees only what follows; `rig.shutdown()` finishes every stream (as archiving the channel does). Deliberate break: hand the same continuation to every subscriber → the second stream is empty.
...
/// `ClaudeProcess`; the seam exists so the one row no real child can produce (wedged: a `TerminationReport` whose `exit` is `nil`), the
...
public protocol ProcessHandle: Sendable {
    ...
    func terminate() async -> TerminationReport        // the exit plus this epoch's escalation steps
}
...
public final class LiveProcessHandle: ProcessHandle {
    ...
    public let diagnostics: CapturingDiagnostics               // the sink the factory passed as ClaudeProcess.init(diagnostics:)
    public init(_ process: ClaudeProcess, diagnostics: CapturingDiagnostics) { self.process = process; self.diagnostics = diagnostics }
    ...
    public func terminate() async -> TerminationReport {
        TerminationReport(exit: await process.terminate(), steps: diagnostics.steps(for: process.epoch))
    }
    /* every other member forwards as `await process.<member>` */
}
/// Installed by every `ProcessFactory` (production and the rig's) as `ClaudeProcess.init`'s `diagnostics:` argument
/// (`main` ClaudeProcess.swift:49-51). Forwards every event to the fleet's ClaudeWire sink and keeps the `step` of each
/// `.terminateEscalated(step:epoch:)` under its epoch. `ClaudeProcess.terminate()` returns only `ExitStatus?`
/// (ClaudeProcess.swift:497); the steps exist nowhere else.
public final class CapturingDiagnostics: DiagnosticsSink, @unchecked Sendable {   // a lock around `steps`
    public init(forwardingTo sink: any DiagnosticsSink)
    public func record(_ event: DiagnosticEvent)               // forward; on .terminateEscalated append the step under its epoch
    public func steps(for epoch: ProcessEpoch) -> [String]     // in the order recorded
}
public typealias ProcessFactory = @Sendable (ProcessEpoch, LaunchConfiguration) -> any ProcessHandle
```

The production factory (the `Fleet`'s default when `init(factory: nil)`, Task 9) is `{ epoch, launch in let sink = CapturingDiagnostics(forwardingTo: wireSink); return LiveProcessHandle(ClaudeProcess(<the existing arguments>, diagnostics: sink, capture: nil), diagnostics: sink) }` with `wireSink` the fleet's ClaudeWire sink; the rig's factory does the same over its `RecordingDiagnostics`. No factory passes `NullDiagnostics` or the fleet's sink straight to `ClaudeProcess`, or the wedged trace is empty.
...
- State: `state: ChannelState`, `epoch: ProcessEpoch` (starts `.first`, `.next()` on every spawn), `process: (any ProcessHandle)?`, `turnRunning`, `pendingDecisions` (kept on `state.pendingDecisions: [PendingDecision]` in ask order; the eligibility input takes its `count`), `queuedInput: [UserInput]`, `pendingHatch: PaneRequest?`, `crashCount`, `dormantTimer: Task<Void, Never>?`, `subscribers: [UUID: AsyncStream<WireEvent>.Continuation]` (the fan-out `events()` returns). Activity: `noteActivity()` runs on every send, every engine frame and every answer, fire-and-forget for the pump but ordered per supervisor: `activityTask = Task { [prev = activityTask] in await prev?.value; await fleet.noteActivity(key) }`; `drainActivity()` awaits the chain's tail (the LRU test calls `rig.drainActivity()`, which drains every supervisor, between its batches so the stamps land in push order). The counter's `activityClock` stamps the key, no per-supervisor sequence exists, and `ChannelState.lastActivity: Date` stays for display only.
...
- `answer(_ id: RequestID, _ answer: InboundAnswer)` (the facade's `perform(.answer(id, answer), on:)`): `guard pendingDecisions.contains(where: { $0.id == id }) else { throw LifecycleError.decisionGone(id) }` (unknown, cancelled, already answered, asked by an older epoch's process, or emptied by an exit); remove the entry and push eligibility *before* the write; then `do { try await process.answer(id, answer); noteActivity() } catch { diagnostics.record(.answerWriteFailed(id: id, reason: String(describing: error))); throw LifecycleError.answerFailed(id, reason: String(describing: error)) }`. The order mirrors `ClaudeProcess.answer`, which removes `pendingInbound[id]` before `writeAnswer` (`main` ClaudeProcess.swift:396-399): after a failed write the id is gone in ClaudeWire and every retry would throw `unknownRequest`, and a failed write means the pipe is gone and an exit follows. The channel, the shell's overlay and the Activity view answer through this and nothing else.
...
- `terminateOrWedge(during action: TerminatingAction) async -> TerminateOutcome` (`.exited(ExitStatus)` or `.wedged(EscalationTrace)`) is the only call site of `process.terminate()`: a report whose `exit` is `nil` applies `.terminateReturnedNil(during: action)` to `.wedged`, records `EscalationTrace(steps: report.steps, pid:, epoch:)`, and every caller returns at once on `.wedged` without running what would have followed an exit. `reap()`: `terminateOrWedge(during: .reap)`; `.wedged` → stop (Task 5 fills the wedged state); `.exited` → `apply(.dormantTimerFired, to: .dormant)`, `process = nil`, `fleet.release(key)`.
...
- Modify: `FleetKit/Sources/FleetSessions/Ownership/OwnershipCheck.swift` (adds the `onReleased` seam the preempt tests use: a closure `awaitRelease` runs after it observed `.released`, nil in production)
- Modify: `FleetKit/Sources/FleetSessions/Lifecycle/FleetCapCounter.swift` (the eviction path)
- Test: `FleetKit/Tests/FleetSessionsTests/LifecycleRowTests.swift` (the remaining twelve rows)
...
- Produces: `adopt()`, `sendToBackground()`, `openInTerminal() -> PaneRequest`, `attach(job:)`, `logs(job:)`, `paneExited(_:)`, `reopen()`, Contended handling, the wedged state, the reservation-based eviction path, the post-release ownership recheck (`handoffPreempted`); after this task every scenario is landed but the four `terminateExhausted` ones during `.restart` (Task 6) and `.logout` (Task 8).
...
- `testTerminateReturningNilMarksTheChannelWedgedAndReopenWaitsForNoHolder` (row `terminateExhausted`, `during: .reap`): `rig.useScriptedHandle(terminateReturns: TerminationReport(exit: nil, steps: Self.wedgedSteps))` where `static let wedgedSteps = ["graceful_phase_deadline_exceeded", "end_session", "stdin_close_requested", "SIGTERM", "SIGKILL", "exit_not_observed"]` (ClaudeProcess's own step names, `main` ClaudeProcess.swift:511-582; the full vocabulary adds `never_launched` and `no_live_child_to_signal`); from ready, `reap()` → state `.owned(.dormant)` with `wedged?.steps == Self.wedgedSteps` exactly and the handle's `pid` and `epoch`, `liveCount` still counts it, `DormantEligibility` for the channel reports `.blocked(.wedged)`, and a later `send` does *not* spawn; `reopen()` with a foreign holder present spawns nothing; `reopen()` with none spawns and clears `wedged`. Stated in the test's doc comment: this row runs against a scripted handle because SIGKILL cannot be refused. Deliberate break: treat `nil` as an exit → the channel respawns on `send`.
- `testTerminateReturningNilDuringSendToBackgroundRunsNoVerb` (`during: .sendToBackground`): ready on the scripted handle; `sendToBackground()` throws `LifecycleError.wedged(trace)`; the runner recorded no `--bg --resume`; `FleetKitKeys.ownJobShorts` unchanged; `state.wedged?.steps == Self.wedgedSteps`; state wedged. Deliberate break: run the verb before checking the outcome → the runner shows `--bg`.
- `testTerminateReturningNilDuringOpenInTerminalReturnsNoPaneRequest` (`during: .openInTerminal`): `openInTerminal()` throws `LifecycleError.wedged(trace)`; no `.paneRequest` diagnostic; `pendingHatch == nil`; state wedged with `wedged?.steps == Self.wedgedSteps`. Deliberate break: build the request before terminating → a request is returned for a process that is still alive.
...
- `testCapEvictsTheLeastRecentlyUsedEligibleChannelAndRefusesWhenNoneIsEligible` (row `capReached`): six supervisors ready on scripted handles (`rig.useScriptedHandle`, so frames can be pushed); recency is driven through the supervisors and never by assigning values: channel 3 receives ten frames first (pushed through `rig.scriptedHandles[2]`), `rig.drainActivity()`, then every other channel receives one frame, `rig.drainActivity()` again, then nothing more reaches channel 3; a seventh `open()` → channel 3 is reaped (its `terminate` observed), the counter received `.evicted`, and the seventh spawns; then make all six ineligible (each with a running `MirrorEntryStandIn`) and open an eighth → `LifecycleError.capReached(live: 6)`, no `terminate` on any, every state's `liveCount == 6` and `headerNote == .capReached(live: 6)`. Also: a wedged channel among the six is never the eviction pick even when least recent. A dormant channel is never a victim: it holds no slot (`reap()` released it), which is why the table has no dormant `capReached` scenario. Deliberate breaks: pick by recency without the eligibility filter → the ineligible half evicts; keep a per-supervisor counter instead of the fleet's `activityClock` → channel 3's count of ten outranks the others and a wrong channel is reaped.
...
- `testAHolderAppearingAfterReleaseAndBeforeLaunchPreemptsSendToBackground` (row `handoffPreempted`, six scenarios): `rig.onReleased`, the `OwnershipCheck` seam that runs when `awaitRelease` returns `.released` (nil in production), writes a registry record for the session into the scripted holder files; from ready, `sendToBackground()` throws `LifecycleError.heldElsewhere(holders)`, the runner recorded no `--bg --resume`, `FleetKitKeys.ownJobShorts` is unchanged, and the state is `.foreignLive(.usersTerminal)` with `banner == .heldElsewhere(holders)` when the record is a foreign live pid, `.backgroundJob` when the hook also adds a roster entry (a job holder), `.owned(.contended)` with `.contended(holders)` when the record carries our own pid; the three again from dormant (`reap()` first: there is no process and no wait, so the recheck is the only check). Deliberate break: skip the recheck → the verb runs and a second writer exists.
- `testAHolderAppearingAfterReleaseAndBeforeLaunchPreemptsOpenInTerminal` (row `handoffPreempted`, the same six): the same hook; `openInTerminal()` throws `heldElsewhere`, no `PaneRequest` is returned, no `.paneRequest` diagnostic, `pendingHatch == nil`, and the state and banner are as above for each holder kind from ready and from dormant. Deliberate break: skip the recheck → a request is returned and the panel spawns beside the holder.
...
- `sendToBackground()`: requires owned; `terminateOrWedge(during: .sendToBackground)` (`.wedged` → throw `LifecycleError.wedged(trace)`, nothing else runs); `awaitRelease(previous: ownHolder)` where `ownHolder` is the registry record whose pid is our child's (looked up before terminating); then, immediately before the verb, `ownership.beforeSpawn(session:)` once more (from dormant, where there is no process and no wait, this is the only check): a holder → no verb; `OriginResolver.resolve` over the holders decides `.foreignLive(.usersTerminal)`, `.backgroundJob` or, for an own pid, `.owned(.contended)`; `banner = .heldElsewhere(set)` (`.contended(set)` for contended); `apply(.holderAppearedBeforeLaunch)` to that state; `throw LifecycleError.heldElsewhere(set)`; clean → `verbs.backgroundResume(id, cwd)` → `JobShort`; store it in `FleetKitKeys.ownJobShorts`; `verbs.agentsJSON()` and the roster must list it, else `LifecycleError.verbFailed(verb: "--bg --resume", exitCode: 0)` with a diagnostic `.jobNotListedAfterBackground`; `apply(.ownedSendToBackground)`.
- `openInTerminal()`: requires owned; `terminateOrWedge(during: .openInTerminal)` (`.wedged` → throw `LifecycleError.wedged(trace)`, no request); `awaitRelease`; the same recheck as `sendToBackground()` (a holder → no request, the holder's origin and banner, `throw LifecycleError.heldElsewhere(set)`; the window between a returned request and the panel's spawn is accepted, spec Decision Log 2026-09-05); build `PaneRequest(id: UUID(), executable: launch.binary, arguments: ["--resume", key.session.description], cwd: launch.cwd, environment: launchTemplate.childEnvironment(over: environment, configHome: configHome), purpose: .hatch(key.session))`; `pendingHatch = request`; `apply(.ownedOpenInTerminal)`; return it. `attach(job:)`/`logs(job:)`: `PaneRequest(executable: binary, arguments: ["attach"|"logs", short.rawValue], cwd: job.cwd ?? launch.cwd, environment: same, purpose: .attach|.logs)`, no state change.
...
- `testAForkWhoseIdentityCollidesWithAnOwnedSessionYields`: two supervisors A (ready on session S, a real replay) and F (a fork of another session, on the scripted handle with a canned handshake); the test pushes `.sessionIdentityResolved(S, F.epoch)` through F's event stream (a fork's identity resolves from the first `auth_status` frame and never from the initialize response, `main` `ClaudeProcess.swift:271-287`, so `FAKE_CLAUDE_INIT`, which patches only the init response, cannot script it); on that event F's post-handshake check sees A's pid, which is not F's own → F terminates its process (the scripted `terminate` observed), `fleet.rollback` released F's provisional reservation (no key of F's in the counter), F is `.owned(.contended)` with the banner, F never published `.ready`, the transition recorded is the `connectingFoundHolder` row `(.connecting, .handshakeFoundHolder, .contended)`, the facade does not re-index F over A, and A's process and state are untouched. Deliberate breaks: skip the post-handshake check on re-key → two supervisors own S; publish `.ready` on the handshake → the collision runs from `.ready`, where `handshakeFoundHolder` has no row, and `transitionNotInTable` is recorded.
- `testAForkReleasesItsSlotUnderTheResolvedKey`: fork on a ready channel (scripted handles); push `.sessionIdentityResolved(N, epoch)` with a fresh id N; the clean check → `fleet.rekey(provisional, to: N)` ran once, the counter's `live` holds N and never held the provisional key after the rekey (assert on the counter's recorded decisions), `.ready` published once; `reap()` → a following `acquire` is `.granted` with no eviction. Deliberate break: leave the slot under the provisional key → `reap()` releases N, which the counter never held, the slot leaks, and the next `acquire` evicts or refuses.
...
- Fork: `fork(at point: ForkPoint?) -> ChannelKey` creates a new `ChannelSupervisor` through a factory closure the facade injects (`spawnSibling`), with `session: point.map { .forkFrom(source, at: $0) } ?? .resume(source, fork: true)`; `ClaudeProcess` sets `identity = .awaitingFork(from: source, provisional:)` for both, and the supervisor keeps it on `state.identity`. While `identity == .awaitingFork` the supervisor does not apply `handshakeClean` after `spawn(handshakeTimeout:)` returns: it stays `.connecting`, skips the provisional-id ownership check (meaningless for a random id), and frames still fan out to subscribers. On `.sessionIdentityResolved(id, epoch)` for the current epoch, in order: `ownership.afterHandshake(session: id, ownPID:, epoch:)` against the *resolved* id (a holder takes the existing `connectingFoundHolder` rows: terminate, `fleet.rollback`, foreign or contended, exactly as a post-handshake holder does); clean → `fleet.rekey(provisional, to: id)` (one counter turn moving the `reserved`, `live`, `lru`, `eligibility` and `wedged` entries), `key = ChannelKey(configHome:, session: id)`, `identity = .known(id)`, the facade re-indexes, `apply(.handshakeClean, to: .ready)`, `armDormantTimer()`, publish. An identity that does not resolve within `handshakeTimeout` fails the spawn on the existing handshake-timeout path.
...
- `testAcceptRecordsTheHashAndDoesNotRepeatUntilTheEntryChanges`: accept `d` → the store holds `(root, "d", hash)`; re-evaluate → `d: approved(byAcceptance)`; change `d`'s command in `.mcp.json` → `d: pending` again, and so does an `env` change (the hash covers the whole raw entry). Deliberate break: key by name alone → the changed entry is still approved.
- `testProjectServersOfEveryTransportAreListedAndConsentIsPerName`: a `.mcp.json` declaring `s` (`{"command": "npx", "args": ["-y", "@example/s"]}`), `h` (`{"type": "http", "url": "https://example.invalid/mcp"}`) and `e` (`{"type": "sse", "url": "https://example.invalid/sse"}`), the three shapes bundle 38223 admits → three pending `ProjectMCPServer`s with transports `.stdio(command: "npx", arguments: ["-y", "@example/s"])`, `.http(url:)` and `.sse(url:)`; accept `h` → `h: approved(byAcceptance)`; change `h`'s url → `h: pending` again; decline `e` → the local store's `disabledMcpjsonServers == ["e"]` (names only, the same array for every transport); an entry `{"type": "websocket", "url": ...}` is `.other(type: "websocket")`, listed and pending like the rest. Deliberate break: hash `command` and `args` only → the url change leaves `h` approved.
...
- `testDeclineRefusesAStagingDirectoryInsideTheConfigHome`: `.claude` is a real directory but `.claude/.cc-writes` is a symlink into the scratch config home → the `O_NOFOLLOW` open refuses `symlink` and nothing under the config home changes (witnessed by hashing it); and a project whose `.claude` resolves through a symlink into the config home is refused `symlink` (the `openat(root, ".claude", O_DIRECTORY | O_NOFOLLOW)` fails with `ELOOP` before any `F_GETPATH` runs on it), before any `mkdirat`; `insideConfigHome` is proven by `testDeclineRefusesAStoreInsideTheConfigHome` and the ancestor-swap test. Deliberate break: check `insideConfigHome` on the store file only → the staging write lands in the config home.
...
`Preconditions/ManagedSettingsReader.swift`: `isPending(configHome:) -> Bool`: `remote-settings.json` absent → false; present and `remote-settings-consent.json` parses to an object whose `approvedHash` (or, failing that key, any string field) equals `ContentHash.sha256Hex` of the payload file's bytes → false; anything else → true. The consent key name is a delegated unknown; the reader's doc comment says so and names the spec section.
...
- Produces: `ActivityQuery.rows(...)`, `ActivityRow`, `FileFleetDiagnostics`, `public actor Fleet: LifecycleAPI` with `init(configHome:, environment:, binary:, store:, diagnosticsDirectory:, clock:, factory: ProcessFactory? = nil, runner: any ProcessRunner = FoundationProcessRunner())`, `events(of:)` (the supervisor's fan-out, nil when the key has no owned supervisor), `perform(.answer(id, answer), on:)` delegating to the supervisor's `answer`, `jobs() -> [JobEntry]` and `performJob(_:_:)`.

- [ ] **Step 1: Write the failing tests**

`ActivityQueryTests.swift`: a table of inputs → rows: a pending decision → `.decision(key, requestID)`; two pending on one channel → two decision rows in ask order (`ChannelState.pendingDecisions` is the source, one row per entry); a `result` with `is_error` → `.failedResult`; `permission_denials` entries and a `system/permission_denied` → `.permissionDenied`; a `rate_limit_event` with `status: "allowed"` and `overageStatus: "rejected"`, `overageDisabledReason: "org_level_disabled"` → **no** refusal row and a banner row of kind `.rateLimitInfo` (deliberate break: key on `overageStatus` → a refusal row appears); `status: "rejected"` → `.rateLimitRefused`; an `auth_status` with `error` → `.authProblem`, a healthy one clears it; a running mirror entry → `.agentRunning`, a failed `task_notification` → `.agentFailed`. Each row carries the `ChannelKey` and, when it exists, the item uuid.

`ImportGraphTests.swift`: grep every `import` line under `FleetKit/Sources/FleetSessions` and assert the set of imported modules is a subset of `["Foundation", "Darwin", "AfleetCore", "ClaudeWire", "WireFrames", "WireTransport", "WireEnvironment", "WireMCP", "WireDiagnostics", "FleetTimeline", "FleetSessions", "CryptoKit"]` and contains at least `AfleetCore` and `ClaudeWire` (the floor that proves the grep found the files). Deliberate break: import `Workbench` in a scratch file → the set gains a name.

`FleetFacadeTests.swift`: `Fleet` built over a `ScratchConfigHome` with the fake-claude factory: `states()` lists a channel after `open(key)`; `updates` publishes; `preconditions(for:)` runs; `events(of: key)` handed to two subscribers yields the replay's frames to both alike and returns nil for a key with no supervisor; `perform(.answer(id, allow), on: key)` for an id no process asked throws `decisionGone`; `openInTerminal` returns a request and `paneExited` re-adopts (facade-level replay of Task 5's row); `declineProjectServers` writes under a temporary project; `jobs()` over scripted holder files lists an exec job (`sessionID == nil`) and a conversation job (`sessionID` set, the same session `states()` shows as a `.backgroundJob` channel); `performJob(.stop, short)` → the runner recorded `stop <short>` and the entry left the roster, `.respawn` and `.remove` run `respawn` and `rm`, and a non-zero exit throws `verbFailed(verb:exitCode:)`; the diagnostics file under a temporary directory has one JSON line per event with the key set `{event, ...structural}` and no key named `path`, `environment`, `stdout`, `record` (assert on key names). Deliberate break: log the pane request's environment → a forbidden key appears.
...
`Activity/ActivityQuery.swift`: `ActivityRow {key, kind: Kind, itemUUID: String?, text: String}` with `Kind = decision(RequestID), notification, failedResult, permissionDenied, rateLimitRefused, rateLimitInfo, authProblem, agentRunning(String), agentFailed(String)`; `rows(states: [ChannelState], mirrors: [ChannelKey: [any TaskMirrorReading]], recent: [ChannelKey: [Frame]]) -> [ActivityRow]` pure; decision rows come from each state's `pendingDecisions`, one per entry in ask order.

`Fleet.swift`: the facade actor owning `[ChannelKey: ChannelSupervisor]`, the `FleetObserver`, `FleetCapCounter`, `StateStore`, `CLIVerbs` (environment composed once through `LaunchConfiguration(binary:cwd: configHome.root, session: .new(SessionID())).childEnvironment(over:configHome:)`), `SpawnPreconditions`, `CommandRouter`, `RefusalInterceptor`, `LogoutPlan`, `spawnBarrier`; implements every `LifecycleAPI` method by delegating to the supervisor for the key (`events(of:)` returns `await supervisor.events()` or nil; `.answer` is forwarded to `supervisor.answer`; the facade holds no process and no stream of its own), creating one on first use with `isRecent` supplied by the caller (`open(key, recent:)` is the facade's extra entry point; the protocol's `perform(.open)` uses the stored recency); `updates` merges every supervisor's stream; a fork re-keys the map on `sessionIdentityResolved` (only after the supervisor's clean check, Task 6); `jobs()` reads the roster through the `FileHolderReader` (reconciled with `agents --json` when read) into `JobEntry`s, exec jobs included; `performJob(verb, short)` runs `verbs.stop`, `verbs.respawn` or `verbs.remove` and throws `verbFailed` on a non-zero exit (a job is keyed by short, not by session, which is why these left `perform(on:)`).
...
- `testAnExecJobIsListedInJobsAndStopRemovesIt`: `skipUnlessLive`; `budget.run(turns: 0)`; `fleet.verbs.backgroundExec("sleep 60", cwd: trustedDir)` → `short`; poll `jobs()` up to 5 s for an entry with that `short`; record whether its `sessionID` is nil (the delegated unknown: an exec job's `state.json` may carry no `sessionId`) as a one-line `XCTContext.runActivity` note with the boolean only; `performJob(.stop, short)`; poll `jobs()` until the short is gone. Deliberate break: list only jobs that name a session → the exec job never appears.
- `testADeclinedProjectServerIsNotSpawnedByTheEngine` (G3's engine-side proof; zero cost): `skipUnlessLive`; `budget.run(turns: 0)`; in a trusted scratch directory, write a `.mcp.json` declaring one stdio server `marker` (stdio because only a spawned command leaves a marker; an http or sse entry is listed and gated identically, with nothing on disk to witness) whose command is `/bin/sh -c 'touch <dir>/marker-<uuid>; exec sleep 30'`; `acceptProjectServers([marker], project: dir)` through the facade (the store-only accept, so Task 7's consent gate does not stop A at `.consentNeeded`; the marker then proves the engine's own promotion, not the gate's); launch A through the `Fleet` with no decline on record (`--max-turns 1` through `budget.launch(_:maxTurns: 1)`) → handshake, `MCPStatus`, `witnessZeroCost`, terminate; assert the marker file exists (the headless path promoted the pending server and spawned its command, parent §6.12) and remove it; then `LocalSettingsStore.decline(["marker"])` through the facade's consent verb (the decline outranks the recorded acceptance, Task 7's precedence); launch B identically → handshake, `MCPStatus`, `witnessZeroCost`, terminate; assert no marker file exists and that the project tree differs from before only by `.claude/settings.local.json`; record both `mcp_status` answers as `XCTContext` notes carrying server counts and status words only (the shape for a rejected server is unrecorded, so it is not asserted). Deliberate break: skip the decline write → B spawns the marker too.
- `testTwoOverlappingScenariosRunOneAtATimeWithAtomicAccounting` (a unit test over a fresh `LiveBudget` built with its own two-turn ceiling, independent of the suite's four; zero cost, runs without the live flag): two `budget.run(turns: 1)` bodies started with `async let`, each recording entry and exit on a shared counter and suspending on a continuation the test resumes; at no point are two bodies inside at once, the second body starts only after the first has returned, and a third `run(turns: 1)` issued while the first two are queued is refused with the budget skip because both turns were reserved at call time, not at body start. Deliberate break: account the turn when the body returns → the third call is admitted.
- `testAdoptingAConversationJobResumesItOwnedAndSendsItBack`: `skipUnlessLive`, `skipUnlessTurns`; `budget.run(turns: 2)` (a resumed job may run a turn); `claude --bg --model claude-haiku-4-5-20251001 --max-turns 1 "Reply with exactly: pong"` through the runner (`claude --bg` forwards `--max-turns`, bundle 824274); poll `jobs()` for the job; `perform(.adopt)`; assert the runner recorded `stop`, the job left the roster, the channel became `.owned(.ready)` under the job's `sessionId` (the resume carries `--max-turns 1` through `budget.launch(_:maxTurns: 1)`), and the post-handshake check found no holder; then `perform(.sendToBackground)` → the runner recorded `--bg --resume <id>`, `jobs()` lists a new job for the session (`sessionID == id`), the channel is `.backgroundJob`; then `performJob(.stop, newShort)` to clean up. Two short turns at most.
- `testTheWriteAllowlistHoldsAcrossHooksABackgroundShellASubagentAndRelocation`: `skipUnlessLive`, `skipUnlessTurns`; `budget.run(turns: 2)` (the prompt and the engine's automatic follow-up turn after the background shell's `task_notification`; the launch carries `maxTurns: 12` through `budget.launch(_:maxTurns:)`: Bash, Task, the subagent's own calls and the reply are agentic turns inside the one prompt); witness before; open a new owned channel in the trusted directory with the Notification hook registered (C2's default `InitializeConfiguration`) and subscribe to `fleet.events(of: key)` before sending; send one `haiku` prompt: `Run "sleep 5 &" as a background shell (run_in_background), then use an Explore subagent to list this directory, then reply "done".`; rule the decision flow from the subscribed stream: on the first `can_use_tool` request hold it until a `hook_callback` request with `callback_id == "afleet.notification"` arrives or 25 s pass (the Notification hook fires only while that ask stays unanswered, about 9.6 s into C1's `notification-hook` fixture, and the callback is itself a surfaced request); answer the hook through `perform(.answer(id, .hookContinue(HookOutput(fields: ["continue": .bool(true)]))))`, the bytes the fixture records as the hook's answer (frames 44-45), then allow the held permission; allow every later request at once. After the prompt's `result`, wait (bounded 60 s) for the shell's `task_notification` and the automatic follow-up turn's `result` (C1's `background-shell` fixture shows that turn: `task_started` L44, first `result` L62, `task_notification` L66, second `result` L86; the project rule forbids closing a channel with a running task); then `/cd` into a second trusted scratch directory (`set_cwd`) and wait for its answer; take the live witness reading now, while the child still runs; terminate; take the final reading. Assert from the channel's events that the prompt's actions ran, each named on failure: a `system/task_started` with `task_type == "local_bash"` (the background shell, as C1's `background-shell` fixture records), a `system/task_started` with `task_type == "local_agent"` (the subagent, as `explore-depth-1` records) and an inbound `hook_callback` request with `callback_id == "afleet.notification"` (the Notification hook, as `notification-hook` records); a missing one fails with its kind named; the hook was answered before the permission (order in the recorded answers); both `result`s are `success` (an `error_max_turns` subtype fails naming it). Both readings must be fully explained, `unexplained(difference).isEmpty` for each with the offending relative paths in the message; the live reading must touch `sessions/` (the child's own record exists only while it runs) and the final reading `projects/` (the transcript), the floor on "the reading looked at the right tree". Deliberate break: remove `projects/` from the allowlist → the transcript is unexplained in the final reading (it is always written, so this is the discriminating demonstration and is run once). `tasks/` stays on the list as inherited from C2's observed set but is no demonstration: a background shell's output lives under the engine's temp artifacts tree, `<artifacts>/<slug>/<session>/tasks/<id>.output` (`Tools/probe/fixture.py:23,217`), not under the config home.
...
- No open item remains from v1: the fork-point flags landed as C2's `SessionStart.forkFrom` (`main` `13c9ad4`) and Task 6 uses them with no skip. Two steps in Task 8 run against scripted `fake-claude` answers because the corpus lacks the frames (the `rewind_files` apply and the completed `claude_oauth_wait_for_completion`), and each test says so; a completed login cannot be recorded without a real OAuth flow, and the rewind apply was never captured. Two facts G5 records rather than asserts: the `mcp_status` shape for a rejected server, and whether an exec job's roster entry names a session (`JobEntry.sessionID`).
